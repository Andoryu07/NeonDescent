    ...
    public String execute(MapState mapS, Player player) {
        if (command != null) {
            ...
            if (player.getHealth() <= 0) {
                dead = true;
                line += "\nSystem failure. You are terminated.";
                return line;
            }
            switch (command){
                case "west":
                ...
                case "south":
                    ...
                    break;
                default:
                    return "Navigation error: Invalid direction.";
            }
            if (id != 0) {
                mapS.setCurrentRoom(mapS.getMap().get(id));
                return line + mapS.getCurrentRoom().roomDescription();
            } else {
                return "No accessible corridor detected in that direction.";
            }
        }
        return "Navigation error: Direction not specified.";
    }
